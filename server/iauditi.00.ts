...
export async function createCompany(company: InsertCompany) {
    ...
    try {
        ...
        if (error.code === '23503') {
            message = "Erro de integridade: O usuário associado não existe no banco de dados.";
        } else if (error.code === '23505') {
            message = "Erro de duplicidade: Já existe um registro com este CNPJ/CPF.";
        }

        throw new Error(`[DEBUG-DB-ERR-${error.code || 'NO-CODE'}]: ` + (error.detail || message));
    }
}
